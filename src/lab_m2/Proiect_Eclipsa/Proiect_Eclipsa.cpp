#include "lab_m2/Proiect_Eclipsa/Proiect_Eclipsa.h"
...
Proiect_Eclipsa::Proiect_Eclipsa()
{
    angle = 0;
...
void Proiect_Eclipsa::Init()
{
...

    {
        ...
        mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "primitives"), "sphere3.obj");
        meshes[mesh->GetMeshID()] = mesh;
    }
    ...
    TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "Planet_Earth.jpg");
    TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "Earth_atmosfera.jpg");
    TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "imagine_transparenta.png");
    TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "moon_texture.jpg");
...
void Proiect_Eclipsa::FrameStart()
{
    ClearScreen();
}
...
void Proiect_Eclipsa::Update(float deltaTimeSeconds)
{
    angle += 0.5f * deltaTimeSeconds;
    earth_orbit_angle += glm::radians(10.0f) * deltaTimeSeconds; // 10 grade/sec
    moon_orbit_angle += glm::radians(20.0f) * deltaTimeSeconds; // 20 grade/sec pentru orbită
...
void Proiect_Eclipsa::DrawScene(Shader* shader)
{
    cout << earth_orbit_angle << "\n";
    int i = 0;
    glm::vec3 position = glm::vec3(-4 + (i % 4) * 2.5, 0, -2 + (i / 4) * 2.5);
    glm::mat4 sphereMatrix = glm::translate(glm::mat4(1), position + glm::vec3(0, 0.3f, 0));
    sphereMatrix = glm::rotate(sphereMatrix, earth_orbit_angle, glm::vec3(0, 1, 0));
    sphereMatrix = glm::scale(sphereMatrix, glm::vec3(10.0f));
    RenderSimpleMesh(meshes["sphere"], shader, sphereMatrix, TextureManager::GetTexture("Planet_Earth.jpg"));
    sphereMatrix = glm::scale(sphereMatrix, glm::vec3(1.0005));
    RenderSimpleMesh(meshes["sphere"], shader, sphereMatrix, TextureManager::GetTexture("imagine_transparenta.png"));
    ...
    float moonDistance = 50.0f; // distanța Lunii față de Pământ
    glm::mat4 moonMatrix = glm::translate(glm::mat4(1), position + glm::vec3(0, 0.f, 0)); // centrul Pământului
    moonMatrix = glm::rotate(moonMatrix, moon_orbit_angle, glm::vec3(0, 1, 0)); // orbita Lunii
    moonMatrix = glm::translate(moonMatrix, glm::vec3(moonDistance, 0, 0)); // deplasăm Luna pe orbită
    moonMatrix = glm::scale(moonMatrix, glm::vec3(2.861f)); // dimensiunea Lunii mai mică decât Pământul
    RenderSimpleMesh(meshes["sphere"], shader, moonMatrix, TextureManager::GetTexture("moon_texture.jpg")); // poți folosi altă textură
}

void Proiect_Eclipsa::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture)
{
...
void Proiect_Eclipsa::DrawFramebufferTextures()
{
...
void Proiect_Eclipsa::RenderTextureScreen(Shader* shader, unsigned int textureID)
{
...
void Proiect_Eclipsa::FrameEnd()
{

}

void Proiect_Eclipsa::LoadShader(const std::string& name)
{
    std::string shaderPath = PATH_JOIN(window->props.selfDir, SOURCE_PATH::M2, "Proiect_Eclipsa", "shaders");
...
void Proiect_Eclipsa::OnInputUpdate(float deltaTime, int mods)
{
...
void Proiect_Eclipsa::OnKeyPress(int key, int mods)
{
    // Add key press event
...
void Proiect_Eclipsa::OnKeyRelease(int key, int mods)
{
    // Add key release event
}


void Proiect_Eclipsa::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    // Add mouse move event
}


void Proiect_Eclipsa::OnMouseBtnPress(int mouseX, int mouseY, int button, int mods)
{
    // Add mouse button press event
}


void Proiect_Eclipsa::OnMouseBtnRelease(int mouseX, int mouseY, int button, int mods)
{
    // Add mouse button release event
}


void Proiect_Eclipsa::OnMouseScroll(int mouseX, int mouseY, int offsetX, int offsetY)
{
    // Treat mouse scroll event
}


void Proiect_Eclipsa::OnWindowResize(int width, int height)
{
