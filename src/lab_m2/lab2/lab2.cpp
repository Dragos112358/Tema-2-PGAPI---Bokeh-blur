...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    for (auto& l : lights)
    {
        ...
        float radians_speed = (2*M_PI/360)*6.0f;
        float rotationRadians = radians_speed*deltaTimeSeconds;
    ...
    // Lighting pass
    {
        ...
        for (auto& lightInfo : lights)
        {
            ...
            RenderMesh(meshes["sphere"], shader, lightInfo.position, glm::vec3(lightInfo.radius * 2.0f));
