...
void EmitVertexForLayer(vec3 position, vec3 color, vec2 uv, vec3 normal, int layer)
{
    ...
    gColor = color;
    gUV = uv;
    gNormal = -mat3(transpose(inverse(Model))) * normal;
...
void main()
{
    ...
    if (instance[0] < no_of_instances - 1)
    {
        for (int i = 0; i < no_of_generated_points - 1; i++)
        {
            ...
            if(length(dT) > 0.0001 && length(dU) > 0.0001)
                normal = normalize(cross(dU, dT));
            else
                normal = vec3(0,1,0); // normal default
            if(length(dT) < 0.0001) dT = vec3(0.001,0,0);
            if(length(dU) < 0.0001) dU = vec3(0,0.001,0);
            normal = normalize(cross(dU,dT));

            if (render_to_cubemap == 0)
            {
                // Normal rendering - single view
                EmitVertexForLayer(point_row1_a, baseColor * brightness1, vec2(u1, v1), normal, 0);
                EmitVertexForLayer(point_row2_a, baseColor * brightness1, vec2(u1, v2), normal, 0);
                EmitVertexForLayer(point_row1_b, baseColor * brightness2, vec2(u2, v1), normal, 0);
                EmitVertexForLayer(point_row2_b, baseColor * brightness2, vec2(u2, v2), normal, 0);
                EndPrimitive();
            }
            else
            {
                ...
                {
                    EmitVertexForLayer(point_row1_a, baseColor * brightness1, vec2(u1, v1), normal, face);
                    EmitVertexForLayer(point_row2_a, baseColor * brightness1, vec2(u1, v2), normal, face);
                    EmitVertexForLayer(point_row1_b, baseColor * brightness2, vec2(u2, v1), normal, face);
                    EmitVertexForLayer(point_row2_b, baseColor * brightness2, vec2(u2, v2), normal, face);
                    EndPrimitive();
                }
            }
