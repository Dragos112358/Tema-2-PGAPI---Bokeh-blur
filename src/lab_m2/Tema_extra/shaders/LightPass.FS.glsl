...
void main()
{
    ...
    wNorm = normalize(texture(texture_normal, tex_coord).rgb); // Ensure normal is normalized

    if (wPos.z >= 50.0 || length(wNorm) < 0.001) { // Example: Skybox depth set to large value or zero normal
        out_color = vec4(0); // Skip lighting for skybox fragments
        return;
    }
