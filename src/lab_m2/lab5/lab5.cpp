...
void Lab5::Init()
{
    ...
    // Load textures
    {
        ...
        TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "fire3.png");
    }
...
void Lab5::ResetParticlesFire(float radius)
{
    ...
    for (unsigned int i = 0; i < nrParticles; i++)
    {
        ...
        data[i].SetInitial(glm::vec4 (pos, 1), glm::vec4 (speed, 0), 0, 0.0f);
    }
...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    if (scene == 3)
    {
        ...
        if (shader->GetProgramID())
        {
            ...
            //TextureManager::GetTexture("fire2.png")->BindToTextureUnit(GL_TEXTURE0 + 1);

            //TextureManager::GetTexture("fire3.png")->BindToTextureUnit(GL_TEXTURE0 + 2);
